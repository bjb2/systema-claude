#!/usr/bin/env python3
"""Session start hook for systema-claude workspaces.

Runs as Claude Code's SessionStart hook (~/.claude/settings.json →
hooks.SessionStart). Reads frontmatter from key files in the workspace
and emits a concise orientation block so the agent doesn't have to
discover the workspace state by reading every file.

Zero dependencies (regex-based YAML parser). Skips on session resume —
...
def find_workspace(data: dict) -> str | None:
    ...
    Priority:
      1. SYSTEMA_CLAUDE_DIR / ORG_DIR env var.
      2. ``cwd`` from hook payload (Claude Code sets this on SessionStart).
      3. Process CWD if it has a CLAUDE.md.
    """
    env_dir = (
...
def parse_frontmatter(filepath: str) -> dict:
    """Best-effort regex YAML frontmatter parser. No PyYAML required."""
    try:
...
def scan_tasks(workspace: str) -> dict:
    """Scan tasks/ folders, return dict by status."""
...
def scan_inbox(workspace: str) -> dict:
    """Scan inbox/ subfolders for pending items."""
    ...
    counts: dict = {v: 0 for v in folder_map.values()}
    counts['other'] = 0
...
def scan_reminders(workspace: str) -> dict:
    ...
    result: dict = {'overdue': [], 'due_today': [], 'due_soon': []}
...
def scan_knowledge_folders(workspace: str) -> dict:
    """Scan knowledge/ folder structure for organisational context."""
...
def main():
    ...
    tasks_by_status = scan_tasks(workspace)
    ...
    current_state_md = os.path.join(workspace, "context", "current-state.md")
    ...
    kb_info = scan_knowledge_folders(workspace)
    if kb_info and kb_info.get('folders'):
        print('### Knowledge Base')
        print('See `knowledge/` for full index.')
        print('')
        print('| Folder | Files |')
        print('|--------|-------|')
    ...
    inbox_counts = scan_inbox(workspace)
    ...
    reminders = scan_reminders(workspace)
    ...
    voice_md = os.path.join(workspace, "context", "voice.md")
